...
local lazyLoad = CreateClientConVar("wire_e2browser_lazy", "0", true, false)
...
local function setTree(dir, parent, lazyLoad )
	...
	for k, v in pairs( pFolders ) do
        ...
        if not lazyLoad then
            setTree( pFolder.FileDir, pFolder, lazyLoad )
        end
	end
...
function PANEL:Init()
    ...
    self.LazyLoad = lazyLoad:GetBool()
...
function PANEL:UpdateFolders()
	...
	setTree( self.startfolder , self.Folders, self.LazyLoad )
	self.Folders.DoClick = function( tree, node )
		...
		if(node.IsFile) then
            ...
            if not node.HasLoaded then
                setTree( node.FileDir, node, self.LazyLoad )
