...
@name Converted Song (via sk89q's midi2e2)
@outputs Index Length Time TimeLength
@persist S:array Start Channels:array ChannelSounds:array ChannelCount

if (first() | duped()) {
    %DATA%
    ChannelCount = 8 # Maximum of 8 sounds
    
    # We must alternate the sound files to reduce the effects of interference
...
class E2CompactVectorWriter(Writer):
    def write(self):
        # We need to sort the notes first
        notes = list(self.data.notes)
        def sort(a, b):
        ...
        self.f.write("S = array(")
        
        # Notes
        index = 0
        for n in notes:
            self.f.write("%svec(%d,%d,%d)" % ("," if index != 0 else "",
                         n.note, n.start, n.end))
            index = index + 1
...
class E2Writer(Writer):
    def write(self):
        vw = E2CompactVectorWriter(self.data, self.f)
        base = e2base.replace("\r\n", "\n").replace("\n", "\r\n").split("%DATA%")
